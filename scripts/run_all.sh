...
python3 "${ROOT_PATH}/src/storage_server.py" "$@" & PIDS+=($!)
sleep 1

python3 "${ROOT_PATH}/src/detect_object.py" "$@" & PIDS+=($!)
sleep 1

python3 "${ROOT_PATH}/src/receiver.py" "$@" & PIDS+=($!)
sleep 1
...
python3 "${ROOT_PATH}/src/sender.py" "$@" & PIDS+=($!)
sleep 1
