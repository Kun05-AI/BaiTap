...
# Xử lý các thư viện lõi, bẫy lỗi import linh hoạt
try:
    import torch
except ImportError:
    pass
...
class AIInferenceService:
    """Nút dịch vụ AI trung tâm: Đọc luồng byte, giải mã hình ảnh, định vị đối tượng và kết xuất dữ liệu."""
    ...
    def _boot_neural_networks(self):
        ...
        if self.use_slicer and HAS_SAHI and self.primary_net:
            try:
                ...
                sys_logger.info("Tích hợp thành công module cắt ảnh lưới SAHI.")
            except Exception as err:
    ...
    def _analyze_payload(self, raw_json):
        ...
        try:
            ...
            # Khôi phục mảng điểm ảnh (Decode Matrix)
            try:
            ...
            outbound_data = {
                ...
                "person_count": len(found_entities),
    ...
    def _run_object_detection(self, bgr_image):
            ...
            try:
                ...
                for item in slicing_output.object_prediction_list:
                    ...
                    if category_info.name == "person" or category_info.id == 0:
                        ...
                        try: certainty = float(item.score.value)
                        except: pass
        ...
        # Dự phòng chạy YOLO nếu SAHI sụp hoặc bị tắt
        if not sahi_success and self.primary_net:
            try:
                ...
                for box in getattr(net_preds, "boxes", []):
                    try:
                    ...
                    if obj_class == 0 and obj_conf >= Config.CONFIDENCE_THRESHOLD:
    ...
    def _export_visual_evidence(self, matrix_img, targets, seq_id, folder_mode, vid_title, interval):
        """Vẽ nhãn đối tượng lên ảnh và xuất ra đĩa cứng định kỳ."""
        try:
            ...
            for t in targets:
                ...
                # Vẽ khung
                cv2.rectangle(canvas, (px, py), (px + pw, py + ph), box_color, 2)

                text_tag = f"Human {pconf:.2f}"
                (txt_w, txt_h), _ = cv2.getTextSize(text_tag, cv2.FONT_HERSHEY_SIMPLEX, 0.6, 2)

                # Nền text
                cv2.rectangle(canvas, (px, max(0, py - txt_h - 8)), (px + txt_w + 4, py), box_color, -1)
                # Ký tự
                cv2.putText(canvas, text_tag, (px + 2, py - 4), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 0, 0), 2)
